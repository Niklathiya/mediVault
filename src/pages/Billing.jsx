  ...
  const savePayment = () => {
    ...
    setBills((prev) =>
      prev.map((b) => {
        ...
        return { ...b, paid: newPaid, status: newStatus, payments: [...(b.payments || []), newEntry] };
      }),
    );
    setModal((m) => {
      ...
      return {
        ...m,
        bill: { ...m.bill, paid: newPaid, status: newStatus, payments: [...(m.bill.payments || []), newEntry] },
      };
    });
    closePayModal();
  };
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 13, color: 'var(--fg-on-light-muted)' }}>
                Patient: <strong style={{ color: 'var(--fg-on-light)' }}>{viewBill.patient}</strong>
              </span>
              <span style={{ fontSize: 13, color: 'var(--fg-on-light-muted)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px' }}>
              ...
              <div style={{ padding: '24px 28px', borderRight: '1px solid var(--border-card)' }}>
                ...
                {/* Totals */}
                <div
                  style={{ marginTop: 16, paddingTop: 16, borderTop: '2px solid var(--border-ui)' }}
                >
                  <div
                    style={{
                      maxWidth: 260,
      ...
      {editOpen && createPortal(
        <div className="modal-backdrop" onClick={closeModal}>
          ...
          >
            ...
            >
              <h2 style={{ margin: 0, fontSize: 18, fontWeight: 600, color: 'var(--fg-on-light)' }}>
          ...
          >
            ...
            >
              <span style={{ color: 'var(--fg-on-light-muted)' }}>
                Total: <strong style={{ color: 'var(--fg-on-light)' }}>{fmt(payModal.amount)}</strong>
              </span>
              <span style={{ color: 'var(--fg-on-light-muted)' }}>
                Paid: <strong style={{ color: '#15803d' }}>{fmt(payModal.paid)}</strong>
              </span>
              <span style={{ color: 'var(--fg-on-light-muted)' }}>
                Balance: <strong style={{ color: '#991b1b' }}>{fmt(payModal.amount - payModal.paid)}</strong>
              </span>
            </div>
