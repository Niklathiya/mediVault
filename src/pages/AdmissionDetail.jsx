...
export default function AdmissionDetail() {
  ...
  const adm = { ...ADMISSION, id: id || ADMISSION.id };
  ...
  return (
    <div style={{ animation: 'mv-fade 200ms ease both' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              style={{ display: 'flex', gap: 20, flexWrap: 'wrap', fontSize: 13, color: C.muted }}
            >
              <span>
                {adm.age} · {adm.blood}
              </span>
              ...
              <span>
                Day <strong style={{ color: C.text }}>{adm.days}</strong>
              </span>
            </div>
          </div>
        </div>
      ...
      <div
        style={{ display: 'grid', gridTemplateColumns: '210px 1fr', gap: 16, alignItems: 'start' }}
      >
        {/* Sidebar nav */}
        <nav
          style={{
              ...
              {section.items.map((item) => {
                ...
                return (
                  ...
                  >
                    <Icon
                      size={15}
                      style={{ flexShrink: 0, color: isActive ? C.primary : C.subtle }}
                    />
                    <span style={{ flex: 1, textAlign: 'left' }}>{item.label}</span>
                    {item.count != null && (
                      <span style={{ fontSize: 11, opacity: 0.55 }}>{item.count}</span>
                    )}
                    {item.dot && (
                      <span
                        style={{
        ...
        <div style={{ animation: 'mv-fade 180ms ease both' }} key={activeTab}>
            ...
            >
              ...
              >
                {[
                  ...
                  ['ESI level', adm.esiLevel],
                  ['Time triaged', '09:15 AM'],
                ].map(([l, v]) => (
                  <div key={l}>
                    <div
                      style={{
          ...
          {activeTab === 'past-history' && (
            ...
            >
              ...
              <div
                style={{
                  padding: '12px 16px',
                  background: 'rgba(78,179,116,0.07)',
          ...
          {activeTab === 'consent' && (
            ...
            >
              ...
              >
                I, <strong style={{ color: C.text }}>{adm.patient}</strong>, hereby give consent to
                the medical staff of BAPS Pramukh Swami Hospital to perform necessary examinations,
                investigations, and treatments as deemed appropriate by the attending physician.
              </div>
              <div style={{ display: 'flex', gap: 12, fontSize: 13, color: C.muted }}>
                <div>
                  <span style={{ fontWeight: 600, color: C.text }}>Signed by:</span> {adm.patient}
                </div>
                <div>
                  <span style={{ fontWeight: 600, color: C.text }}>Date:</span> {adm.admittedOn}
                </div>
              </div>
              <div
                style={{
            ...
            >
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 13, color: C.text, lineHeight: 1.7 }}>
                  Patient presents with severe abdominal pain in right iliac fossa for 2 days,
                  associated with fever and nausea. No vomiting. Bowel habits normal.
                </div>
              </div>
              <div>
                <div
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {[
                    ...
                    ['Respiratory', 'NVBS, clear'],
                    ['Abdomen', "Guarding, tenderness at McBurney's point"],
                  ].map(([s, v]) => (
                    <div
                      key={s}
          ...
          {[
            ...
            <EmptyTab
              label={
                NAV_SECTIONS.flatMap((s) => s.items).find((i) => i.id === activeTab)?.label ??
                activeTab
              }
            />
          )}
