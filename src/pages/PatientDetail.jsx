...
export default function PatientDetail() {
  ...
  const removeItem = (key, itemId) => {
    const itemLabel = key.slice(0, -1) || 'item';
  ...
  const timeline = [
    ...patient.visits.map((v) => ({
      date: v.date,
      label: ensureDmyDate(v.date || v.dateLabel),
      time: v.time || '09:30 AM',
      type: 'Visit',
      icon: ClipboardList,
      color: '#0891b2',
      bg: 'rgba(8,145,178,0.10)',
      title: `OPD Visit — ${v.dept}`,
      detail: `${v.doctor} · ${v.complaint}`,
    })),
    ...patient.prescriptions.map((p) => ({
      date: p.date && p.date.includes('/') ? p.date.split('/').reverse().join('-') : '2026-06-20',
      label: ensureDmyDate(p.date),
      time: p.time || '11:15 AM',
      type: 'Prescription',
      icon: Pill,
      color: '#7c3aed',
      bg: 'rgba(124,58,237,0.10)',
      title: `Prescribed — ${p.drug}`,
      detail: `${p.dosage} · ${p.duration} · ${p.doctor}`,
    })),
    ...patient.labs.map((l) => ({
      date: l.date && l.date.includes('/') ? l.date.split('/').reverse().join('-') : '2026-06-20',
      label: ensureDmyDate(l.date),
      time: l.time || '02:30 PM',
      type: 'Lab',
      icon: FlaskConical,
      color: '#d9a441',
      bg: 'rgba(217,164,65,0.10)',
      title: `Lab — ${l.test}`,
      detail: `Result: ${l.result} (Normal: ${l.normal}) · ${l.status}`,
    })),
    ...patient.admissions.map((a) => ({
      date: a.admittedOn,
      label: ensureDmyDate(a.admittedOn),
      time: a.admittedTime || '04:45 PM',
      type: 'IPD Admission',
      icon: BedDouble,
      color: '#C2410C',
      bg: 'rgba(194,65,12,0.10)',
      title: `Admitted — ${a.ward} Ward · ${a.ipNo}`,
      detail: `${a.admittingDoctor} · Status: ${a.status}${a.dischargedOn ? ' · Discharged: ' + ensureDmyDate(a.dischargedOn) : ''}`,
    })),
  ...
  const printPatient = () => {
    ...
    const pri = iframe.contentWindow || iframe.contentDocument;
    pri.document.open();
    pri.document.write(`<!DOCTYPE html><html><head><title>Patient — ${patient.name}</title>
    ...
    <div style={{ animation: 'mv-fade 200ms ease both' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div
              style={{ display: 'flex', gap: 18, flexWrap: 'wrap', fontSize: 13, color: C.muted }}
            >
              <span>{id}</span>
              <span>
              ...
              <span>
                Blood <strong style={{ color: C.text }}>{patient.blood}</strong>
              </span>
      ...
      {patientAllergies.length > 0 && (
        ...
        >
          ...
          <div>
            <div
              style={{ fontSize: 13, fontWeight: 700, color: '#a13030', letterSpacing: '0.04em' }}
            >
              ALLERGY ALERT
            </div>
      ...
      <div key={tab} style={{ animation: 'mv-fade 180ms ease both' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              >
                ...
                <InfoRow
                  label="Age / Sex"
                  value={`${patient.age} yrs, ${patient.sex}`}
                  icon={User}
                />
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                >
                  ...
                  <InfoRow
                    label="Allergies"
                    value={
                      patientAllergies.length ? patientAllergies.join(', ') : 'No known allergies'
                    }
                  />
                </div>
              </div>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                  {patient.admissions.length} admission(s) on record · click to view full IPD
                  history &amp; case files
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {patient.admissions.length > 0 && (
                  ...
                  >
                    {patient.admissions[0].status === 'admitted'
                      ? 'Currently admitted'
                      : 'Last: ' + fmtDate(patient.admissions[0].admittedOn)}
                  </span>
                )}
          ...
          <div>
            ...
            >
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1.4fr 0.8fr 1fr 0.8fr 1.2fr 76px',
                  padding: '12px 20px',
                  background: C.subtleBg,
                  fontSize: 11,
                  letterSpacing: '0.06em',
          ...
          <div>
            ...
            >
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1.4fr 1fr 1fr 1fr 0.8fr 76px',
                  padding: '12px 20px',
                  background: C.subtleBg,
                  fontSize: 11,
                  letterSpacing: '0.06em',
              ...
              {patient.labs.length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center', color: C.muted, fontSize: 14 }}>
                  No lab results on record. Lab investigations ordered during IPD admission appear
                  in IPD Admissions → Investigations.
                </div>
              ) : (
                  ...
                  >
                    ...
                    <div
                      style={{
                        fontSize: 13,
                        fontWeight: 500,
                        color: lab.status === 'High' ? '#d9a441' : C.text,
                      }}
                    >
                      {lab.result}
                    </div>
                    ...
                    <div>
                      <span
                        style={{
                          ...
                          padding: '3px 8px',
                          borderRadius: 10,
                          background: lab.statusBg,
                          color: lab.statusColor,
                          fontWeight: 500,
                        }}
                      >
                        {lab.status}
          ...
          <div>
              ...
              >
                ...
                >
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                      Latest reading · {patient.vitals[0].date}
                    </div>
                  </div>
            ...
            >
              <div
                style={{
                  display: 'grid',
                  gridTemplateColumns: '1fr 1fr 0.7fr 0.7fr 0.7fr 0.7fr 76px',
                  padding: '12px 20px',
                  background: C.subtleBg,
                  fontSize: 11,
                  letterSpacing: '0.06em',
        ...
        {tab === 'documents' && (
          <div>
            ...
            >
              <div style={{ fontSize: 13, color: C.muted }}>
                Scanned reports, X-rays, consent forms, etc.
              </div>
              <button
                className="btn-primary"
                style={{ fontSize: 13 }}
          ...
          <div>
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>
                Complete Patient Record History
              </div>
              <div style={{ fontSize: 12, color: C.muted, marginTop: 2 }}>
                All visits, prescriptions, lab results, vitals and admissions — in one chronological
                view
              </div>
            </div>
            {timeline.length === 0 ? (
              ...
              <div style={{ position: 'relative' }}>
                {/* Vertical connector line — starts at first dot, ends at last dot */}
                {timeline.length > 1 && (
                  <div
                    style={{
                      position: 'absolute',
                      /* left = date column (110px) + half of dot column (20px/2) - half line width (1px) */
                      left: 120,
                      /* top = paddingTop of dot container (18px) + half dot height (5px) */
                      top: 23,
                      /* bottom same offset from last item's dot center */
                      bottom: 50,
                      width: 2,
                ...
                {timeline.map((ti, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      style={{ display: 'flex', gap: 0, marginBottom: 4, alignItems: 'flex-start' }}
                    >
                      <div
                        style={{
                          width: 110,
                      ...
                      >
                        <div
                          style={{
                            ...
                            background: ti.color,
                            boxShadow: `0 0 0 2px ${ti.color}44`,
                          }}
                        />
                      </div>
                      <div
              ...
              <div style={{ display: 'flex', gap: 16, fontSize: 13 }}>
                <div style={{ color: C.muted }}>
                  ...
                  <strong style={{ color: C.primary }}>
                    {fmt(patient.billings.reduce((s, b) => s + b.amount, 0))}
                  </strong>
                </div>
                <div style={{ color: C.muted }}>
                  Outstanding:{' '}
                  <strong style={{ color: '#991b1b' }}>
                    {fmt(patient.billings.reduce((s, b) => s + (b.amount - b.paid), 0))}
                  </strong>
                </div>
              </div>
            </div>
            ...
            {patient.admissions.length === 0 ? (
              ...
              >
                ...
                <div style={{ fontSize: 14, marginTop: 8 }}>
                  No IPD admissions recorded for this patient.
                </div>
                <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>
                  Click "Admit patient" to create the first admission.
                </div>
              </div>
            ) : (
              <div
              ...
              >
                <div
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '130px 130px 1fr 110px 80px 130px 110px',
                    padding: '11px 18px',
                    background: C.subtleBg,
                    fontSize: 11,
                    letterSpacing: '0.06em',
                ...
                {patient.admissions.map((a) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontWeight: 600, color: C.primary, fontSize: 12 }}>
                        {a.ipNo}
                      </div>
                      <div>
                      ...
                      <div>
                        <div style={{ fontSize: 12, fontWeight: 500 }}>
                          {fmtDate(a.dischargedOn)}
                        </div>
                        {a.dischargedTime && (
                          <div style={{ fontSize: 10, color: C.muted }}>{a.dischargedTime}</div>
                        )}
                      </div>
                      <div>
                        <span
                          style={{
                            ...
                            borderRadius: 10,
                            fontWeight: 500,
                            background: isAdm ? 'rgba(8,145,178,0.10)' : 'rgba(78,179,116,0.10)',
                            color: isAdm ? '#0891b2' : '#15803d',
                          }}
                        >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                  ...
                  <label>
                    ...
                    <select
                      style={inp}
                      value={ef.blood}
                      onChange={(e) => setField('blood', e.target.value)}
                    >
                ...
                <div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
                    <input
                      style={inp}
                      placeholder="Name"
                      value={ef.emergencyName}
                      onChange={(e) => setField('emergencyName', e.target.value)}
                    />
                    <input
                      style={inp}
                      placeholder="Relation"
                      value={ef.emergencyRelation}
                      onChange={(e) => setField('emergencyRelation', e.target.value)}
                    />
                    <input
                      style={inp}
                      placeholder="Phone"
                      value={ef.emergencyPhone}
