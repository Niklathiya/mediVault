...
function runSearch(q) {
  ...
  return {
    patients: SEARCH_POOL.patients.filter(
      (p) => p.name.toLowerCase().includes(lower) || p.id.toLowerCase().includes(lower),
    ),
    admissions: SEARCH_POOL.admissions.filter(
      (a) => a.name.toLowerCase().includes(lower) || a.id.toLowerCase().includes(lower),
    ),
    doctors: SEARCH_POOL.doctors.filter(
      (d) => d.name.toLowerCase().includes(lower) || d.sub.toLowerCase().includes(lower),
    ),
  };
}
...
export default function AdminLayout() {
  ...
  const hasResults =
    results && results.patients.length + results.admissions.length + results.doctors.length > 0;
      ...
      >
        ...
        >
          ...
          <div ref={searchWrapRef} style={{ flex: 1, maxWidth: 560, position: 'relative' }}>
            ...
            <input
              type="text"
              placeholder="Search patients, doctors, admissions, lab results…"
              value={query}
        ...
        {/* Page content */}
        <div
          style={{ flex: 1, overflowY: 'auto', padding: '32px', background: 'var(--bg-canvas)' }}
        >
